...
Template.DeviceSearchModal.onCreated(function DeviceSearchModalOnCreated() {
...
Template.DeviceSearchModal.onRendered(function DeviceSearchModalOnRendered() {
    const modalElement = this.find('#DeviceSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.DeviceSearchModal.helpers({
    filterCount(value) {
    ...
    deviceFilter(type) {
        let deviceFilters = Session.get('deviceFilters');
        if(deviceFilters){
            let identifier =  deviceFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.DeviceSearchModal.events({
    ...
    'click .reset': function (event, instance) {
        ...
        $('#DeviceSearchModal').find('form')[0].reset();
    },

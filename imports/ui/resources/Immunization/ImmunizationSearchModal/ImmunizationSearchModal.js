import "./ImmunizationSearchModal.html";
...
Template.ImmunizationSearchModal.helpers({
    ...
    ImmunizationFilter(type) {
        let ImmunizationFilters = Session.get('immunizationFilters');
        if(ImmunizationFilters){
            let identifier =  ImmunizationFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.ImmunizationSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#ImmunizationSearchModal').modal('hide');

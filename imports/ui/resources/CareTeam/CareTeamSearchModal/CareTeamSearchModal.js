import "./CareTeamSearchModal.html";
...
Template.CareTeamSearchModal.onCreated(function CareTeamSearchModalOnCreated() {
...
Template.CareTeamSearchModal.onRendered(function CareTeamSearchModalOnRendered() {
    const modalElement = this.find('#CareTeamSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.CareTeamSearchModal.helpers({
    filterCount(value) {
    ...
    careTeamFilter(type) {
        let careTeamFilters = Session.get('careTeamFilters');
        if(careTeamFilters){
            let identifier =  careTeamFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.CareTeamSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#CareTeamSearchModal').modal('hide');
    ...
    'click .reset': function (event, instance) {
        ...
        $('#CareTeamSearchModal').find('form')[0].reset();
    },

import "./DiagnosticReportSearchModal.html";
...
Template.DiagnosticReportSearchModal.helpers({
    ...
    DiagnosticReportFilter(type) {
        let DiagnosticReportFilters = Session.get('diagnosticReportFilters');
        if(DiagnosticReportFilters){
            let identifier =  DiagnosticReportFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.DiagnosticReportSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#DiagnosticReportSearchModal').modal('hide');
        ...
        if(decision === 'acceptResults'){
            ...
            Session.set("isFindingDoc", false);
            $('#DiagnosticReportSearchModal').modal('show');
        } else {
            setDocs(res);
            console.log('res---', res);
        }

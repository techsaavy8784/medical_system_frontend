import "./EncounterSearchModal.html";
...
Template.EncounterSearchModal.onCreated(function EncounterSearchModalOnCreated() {
...
Template.EncounterSearchModal.onRendered(function EncounterSearchModalOnRendered() {
    const modalElement = this.find('#EncounterSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.EncounterSearchModal.helpers({
    filterCount(value) {
    ...
    encounterFilter(type) {
        let encounterFilters = Session.get('encounterFilters');
        if(encounterFilters){
            let identifier =  encounterFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.EncounterSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#EncounterSearchModal').modal('hide');
    ...
    'click .reset': function (event, instance) {
        ...
        $('#EncounterSearchModal').find('form')[0].reset();
    },

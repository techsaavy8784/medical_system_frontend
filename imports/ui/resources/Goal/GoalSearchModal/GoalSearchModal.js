...
Template.GoalSearchModal.onCreated(function GoalSearchModalOnCreated() {
...
Template.GoalSearchModal.onRendered(function GoalSearchModalOnRendered() {
    const modalElement = this.find('#GoalSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.GoalSearchModal.helpers({
    filterCount(value) {
    ...
    goalFilter(type) {
        let goalFilters = Session.get('goalFilters');
        if(goalFilters){
            let identifier =  goalFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.GoalSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#GoalSearchModal').modal('hide');
    ...
    'click .reset': function (event, instance) {
        ...
        $('#GoalSearchModal').find('form')[0].reset();
    },

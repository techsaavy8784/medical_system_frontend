import "./MedicationDispenseSearchModal.html";
...
Template.MedicationDispenseSearchModal.onCreated(function MedicationDispenseSearchModalOnCreated() {
...
Template.MedicationDispenseSearchModal.onRendered(function MedicationDispenseSearchModalOnRendered() {
    const modalElement = this.find('#MedicationDispenseSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.MedicationDispenseSearchModal.helpers({
    filterCount(value) {
    ...
    medicationDispenseFilter(type) {
        let medicationDispenseFilters = Session.get('medicationDispenseFilters');
        if(medicationDispenseFilters){
            let identifier =  medicationDispenseFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.MedicationDispenseSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#MedicationDispenseSearchModal').modal('hide');
    ...
    'click .reset': function (event, instance) {
        ...
        $('#MedicationDispenseSearchModal').find('form')[0].reset();
    },

import "./ConditionSearchModal.html";
...
Template.ConditionSearchModal.helpers({
    ...
    conditionFilter(type) {
        let conditionFilters = Session.get('conditionFilters');
        if(conditionFilters){
            let identifier =  conditionFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.ConditionSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#ConditionSearchModal').modal('hide');

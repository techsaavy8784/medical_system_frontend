import "./CoverageSearchModal.html";
...
Template.CoverageSearchModal.onCreated(function CoverageSearchModalOnCreated() {
...
Template.CoverageSearchModal.onRendered(function CoverageSearchModalOnRendered() {
    const modalElement = this.find('#CoverageSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.CoverageSearchModal.helpers({
    filterCount(value) {
    ...
    coverageFilter(type) {
        let coverageFilters = Session.get('coverageFilters');
        if(coverageFilters){
            let identifier =  coverageFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.CoverageSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#CoverageSearchModal').modal('hide');
    ...
    'click .reset': function (event, instance) {
        ...
        $('#CoverageSearchModal').find('form')[0].reset();
    },

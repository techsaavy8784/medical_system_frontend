import "./ProceduresSearchModal.html";
...
const buildEndPoint = (isExpandResults) => {
    ...
    //TODO: after live server test passed renamed it fully to Procedure
    baseURL += "Procedure";
...
Template.ProceduresSearchModal.onCreated(function ProceduresSearchModalOnCreated() {
...
Template.ProceduresSearchModal.onRendered(function ProceduresSearchModalOnRendered() {
    const modalElement = this.find('#ProceduresSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.ProceduresSearchModal.helpers({
    filterCount(value) {
    ...
    proceduresFilter(type) {
        let proceduresFilters = Session.get('proceduresFilters');
        if(proceduresFilters){
            let identifier =  proceduresFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.ProceduresSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#ProceduresSearchModal').modal('hide');
    ...
    'click .reset': function (event, instance) {
        ...
        $('#ProceduresSearchModal').find('form')[0].reset();
    },

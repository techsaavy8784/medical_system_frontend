import "./NutritionOrderSearchModal.html";
...
Template.NutritionOrderSearchModal.onCreated(function NutritionOrderSearchModalOnCreated() {
...
Template.NutritionOrderSearchModal.onRendered(function NutritionOrderSearchModalOnRendered() {
    const modalElement = this.find('#NutritionOrderSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.NutritionOrderSearchModal.helpers({
    filterCount(value) {
    ...
    nutritionOrderFilter(type) {
        let nutritionOrderFilters = Session.get('nutritionOrderFilters');
        if(nutritionOrderFilters){
            let identifier =  nutritionOrderFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.NutritionOrderSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#NutritionOrderSearchModal').modal('hide');
    ...
    'click .reset': function (event, instance) {
        ...
        $('#NutritionOrderSearchModal').find('form')[0].reset();
    },

import "./AllergyIntoleranceSearchModal.html";
...
Template.AllergyIntoleranceSearchModal.onCreated(function AllergyIntoleranceSearchModalOnCreated() {
...
Template.AllergyIntoleranceSearchModal.onRendered(function AllergyIntoleranceSearchModalOnRendered() {
    const modalElement = this.find('#AllergyIntoleranceSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.AllergyIntoleranceSearchModal.helpers({
    filterCount(value) {
    ...
    allergyIntoleranceFilter(type) {
        let allergyIntoleranceFilters = Session.get('allergyIntoleranceFilters');
        if(allergyIntoleranceFilters){
            let identifier =  allergyIntoleranceFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.AllergyIntoleranceSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#AllergyIntoleranceSearchModal').modal('hide');
    ...
    'click .reset': function (event, instance) {
        ...
        $('#AllergyIntoleranceSearchModal').find('form')[0].reset();
    },

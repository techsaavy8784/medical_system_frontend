import "./AccountSearchModal.html";
...
Template.AccountSearchModal.onCreated(function AccountSearchModalOnCreated() {
...
Template.AccountSearchModal.onRendered(function AccountSearchModalOnRendered() {
    const modalElement = this.find('#AccountSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.AccountSearchModal.helpers({
    filterCount(value) {
    ...
    accountFilter(type) {
        let accountFilters = Session.get('accountFilters');
        if(accountFilters){
            let identifier =  accountFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.AccountSearchModal.events({
    ...
    'click .reset': function (event, instance) {
        ...
        $('#AccountSearchModal').find('form')[0].reset();
    },

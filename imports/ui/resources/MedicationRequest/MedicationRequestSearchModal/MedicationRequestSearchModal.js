import "./MedicationRequestSearchModal.html";
...
Template.MedicationRequestSearchModal.onCreated(function MedicationRequestSearchModalOnCreated() {
...
Template.MedicationRequestSearchModal.onRendered(function MedicationRequestSearchModalOnRendered() {
    const modalElement = this.find('#MedicationRequestSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.MedicationRequestSearchModal.helpers({
    filterCount(value) {
    ...
    medicationRequestFilter(type) {
        let medicationRequestFilters = Session.get('medicationRequestFilters');
        if(medicationRequestFilters){
            let identifier =  medicationRequestFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.MedicationRequestSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#MedicationRequestSearchModal').modal('hide');
    ...
    'click .reset': function (event, instance) {
        ...
        $('#MedicationRequestSearchModal').find('form')[0].reset();
    },

import "./MedicationAdministrationSearchModal.html";
...
Template.MedicationAdministrationSearchModal.onCreated(function MedicationAdministrationSearchModalOnCreated() {
...
Template.MedicationAdministrationSearchModal.onRendered(function MedicationAdministrationSearchModalOnRendered() {
    const modalElement = this.find('#MedicationAdministrationSearchModal');
    // $(modalElement).on('hidden.bs.modal', function (event) {
    //     $(this).find('form').trigger('reset');
    // })
});

Template.MedicationAdministrationSearchModal.helpers({
    filterCount(value) {
    ...
    medicationAdministrationFilter(type) {
        let medicationAdministrationFilters = Session.get('medicationAdministrationFilters');
        if(medicationAdministrationFilters){
            let identifier =  medicationAdministrationFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.MedicationAdministrationSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#MedicationAdministrationSearchModal').modal('hide');
    ...
    'click .reset': function (event, instance) {
        ...
        $('#MedicationAdministrationSearchModal').find('form')[0].reset();
    },

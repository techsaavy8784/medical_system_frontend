import "./QuestionnaireResponseSearchModal.html";
...
Template.QuestionnaireResponseSearchModal.helpers({
    ...
    questionnaireResponseFilter(type) {
        let questionnaireResponseFilters = Session.get('questionnaireResponseFilters');
        if(questionnaireResponseFilters){
            let identifier =  questionnaireResponseFilters.find(filter => filter.element === type)
            return identifier?.filters || []
        }
    }
});

Template.QuestionnaireResponseSearchModal.events({
    async 'submit .search-doc-form' (event, instance) {
        Session.set("executeFinding", false);
        event.preventDefault()
		$('#QuestionnaireResponseSearchModal').modal('hide');
